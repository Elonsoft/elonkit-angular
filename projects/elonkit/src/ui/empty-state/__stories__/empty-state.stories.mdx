...
<Meta title='UI/Empty State' decorators={[withKnobs]} />
...
<Canvas source={EMPTY_STATE_STORY_BASIC_SOURCE}>
  <Story name='Basic' height='170px'>
    {{
      ...
      moduleMetadata: {
        imports: [BrowserAnimationsModule, ESEmptyStateModule]
      },
      props: {
        icon: select('icon', ['box', 'chat', 'face', 'file', 'search'], 'box'),
...
<Canvas source={EMPTY_STATE_STORY_BUTTON_SOURCE}>
  <Story name='Button' height='200px'>
    {{
      ...
      `,
      moduleMetadata: {
        imports: [BrowserAnimationsModule, MatButtonModule, ESEmptyStateModule]
      },
      props: {
        icon: select('icon', ['box', 'chat', 'face', 'file', 'search'], 'box'),
        heading: text('heading', 'There are no entities yet'),
...
<Canvas source={EMPTY_STATE_STORY_TYPOGRAPHY_SOURCE}>
  <Story name='Typography' height='170px'>
    {{
      ...
      `,
      moduleMetadata: {
        imports: [BrowserAnimationsModule, EmptyStateStoryTypographyModule]
      },
      props: {
        icon: select('icon', ['box', 'chat', 'face', 'file', 'search'], 'box'),
        heading: text('heading', 'There are no entities yet'),
